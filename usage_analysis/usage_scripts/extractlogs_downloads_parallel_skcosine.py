...
class ProcessLogs:
    def __init__(self):
        ...
        self.SIM_FILE = os.path.join(self.parent_dir, config['DATASOURCE']['sim_file'])
    ...
    def get_Total_Related_Downloads(self,dfmain):
        ...
        sim = cosine_similarity(df_sparse)

        #write to file
        #output = open(self.SIM_FILE, 'wb')
        #pickle.dump(sim, output)
        #output.close()

        m, n = sim.shape
        logging.info("Shape and Bytes: %s %s", str(m), str(sim.nbytes))
        #logging.info('Before-Diagonal-Sim :%s ', (sim.diagonal() == 1).sum())
        if ((sim.diagonal() == 1).sum() != m):
            logging.info('Filling diagonal..')
